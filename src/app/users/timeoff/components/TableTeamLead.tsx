...
const TableTeamLead: React.FC<{ searchQuery: string }> = ({ searchQuery }) => {
  ...
  const teamLeaderTimeOffs = timeOffs?.filter(
    (i) => i.employee_info.teamLeader === sessionId
  );

  const QuerySerach = teamLeaderTimeOffs?.filter(
    (query) =>
  ...
  const paginatedTeamLeader: TimeOff[] = paginate(
    QuerySerach,
    pageIndex,
    pageSize
  );
  ...
  if (teamLeaderTimeOffs?.length === 0) return <AlertNoneData />;
              ...
              >
                ...
                <td>
                  <p className="bg-gray-100 text-center rounded-md shadow-sm p-0.5 ">
                    <FormatDateTimeOff
                      dateData={teamLeader?.createdAt.split("T")[0]}
                    />
                  </p>
                </td>
                <td>
